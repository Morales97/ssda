    ...
    def generate_pseudolabels(self, model, ema, tau=0.9, ignore_index=250):
        ...
        with ema.average_parameters() and torch.no_grad():
            for img_path in self.files[self.split]:
                ...
                pseudo_lbl2 = self.encode_from_trainid_to_id(pseudo_lbl)
                #a = pseudo_lbl
                ...
                #pdb.set_trace()
                print(np.unique(pseudo_lbl2, return_counts=True))
                pseudo_lbl_im = Image.fromarray(pseudo_lbl2.squeeze(0), mode='L')   
